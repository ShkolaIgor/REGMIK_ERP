...
-- Modify existing roles table to allow NULL for permissions column if it exists
DO $$
BEGIN
  IF EXISTS (
...
-- Create base roles with proper conflict handling
DO $$
BEGIN
  -- Insert or update roles one by one to handle conflicts properly
  INSERT INTO roles (name, display_name, description, permissions, is_system_role) 
  VALUES ('super_admin', 'Супер адміністратор', 'Повний доступ до всіх функцій системи', '{}', true)
  ON CONFLICT (name) DO UPDATE SET
    display_name = EXCLUDED.display_name,
    description = EXCLUDED.description,
  ...
  INSERT INTO roles (name, display_name, description, permissions, is_system_role) 
  VALUES ('admin', 'Адміністратор', 'Адміністративні функції та управління', '{}', true)
  ON CONFLICT (name) DO UPDATE SET
    display_name = EXCLUDED.display_name,
    description = EXCLUDED.description,
    permissions = COALESCE(EXCLUDED.permissions, '{}'),
  ...
  INSERT INTO roles (name, display_name, description, permissions, is_system_role) 
  VALUES ('manager', 'Менеджер', 'Управління замовленнями та клієнтами', '{}', true)
  ON CONFLICT (name) DO UPDATE SET
    display_name = EXCLUDED.display_name,
    description = EXCLUDED.description,
    permissions = COALESCE(EXCLUDED.permissions, '{}'),
  ...
  INSERT INTO roles (name, display_name, description, permissions, is_system_role) 
  VALUES ('operator', 'Оператор', 'Обробка замовлень та основні операції', '{}', true)
  ON CONFLICT (name) DO UPDATE SET
    display_name = EXCLUDED.display_name,
    description = EXCLUDED.description,
    permissions = COALESCE(EXCLUDED.permissions, '{}'),
  ...
  INSERT INTO roles (name, display_name, description, permissions, is_system_role) 
  VALUES ('user', 'Користувач', 'Базовий доступ для перегляду', '{}', true)
  ON CONFLICT (name) DO UPDATE SET
    display_name = EXCLUDED.display_name,
    description = EXCLUDED.description,
    permissions = COALESCE(EXCLUDED.permissions, '{}'),
...
-- Assign all permissions to super_admin (with proper error handling)
DO $$
DECLARE
...
BEGIN
  ...
  IF admin_role_id IS NOT NULL THEN
    ...
    SELECT 
        ...
        true
    FROM permissions p
    WHERE p.name NOT IN ('admin_system', 'admin_backup', 'admin_security')
    AND NOT EXISTS (
        SELECT 1 FROM role_permissions rp 
...
BEGIN
  ...
  IF operator_role_id IS NOT NULL THEN
    ...
    AND NOT EXISTS (
        SELECT 1 FROM role_permissions rp 
        WHERE rp.role_id = operator_role_id 
        AND rp.permission_id = p.id
    );
  END IF;
END
...
-- Assign super_admin role to ShkolaIhor user (only if users table exists and is a base table)
DO $$
DECLARE
  super_admin_role_id INTEGER;
BEGIN
  IF EXISTS (
    ...
    IF super_admin_role_id IS NOT NULL THEN
      ...
      AND (role_id IS NULL OR role_id != super_admin_role_id);
    END IF;
  END IF;
END
$$;
