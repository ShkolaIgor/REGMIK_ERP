...
export default function Orders() {
  ...
  const getOrderColumns = (): DataTableColumn<any>[] => {
    return [
      {
      ...
      },
      {
        ...
        cell: (order) => {
          ...
          return (
            <div className="space-y-1">
              {isOverdue && (
                <div className="text-xs bg-red-100 text-red-800 px-2 py-1 rounded border border-red-300 font-medium">
                  ⚠️ ПРОСТРОЧЕНО
                </div>
              )}
              <DueDateButton 
      ...
      },
      {
        id: 'totalAmount',
        header: 'Сума',
        cell: (order) => <div className="font-medium">{formatCurrency(parseFloat(order.totalAmount))}</div>,
        sortable: true,
      },
      {
        id: 'shippedDate',
        header: 'Відвантаження',
        cell: (order) => {
        ...
        cell: (order) => (
          <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
            ...
            <PartialShipmentDialog
              ...
              orderNumber={order.orderNumber}
              trigger={
                <Button variant="outline" size="sm" title="Відвантажити частково">
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  const response = await apiRequest(`/api/orders/${order.id}/check-post-payment`, {
                    method: 'POST'
                  });
                  if (response.success) {
                    ...
                    toast({
                      ...
                      description: response.message || "Перевірка завершена",
                    });
                  }
                } catch (error) {
                  ...
                  toast({
                    title: "Помилка",
                    description: "Не вдалося перевірити оплату",
                    variant: "destructive",
                  });
                }
              }}
              title="Перевірити банківські повідомлення про оплату"
            >
  ...
  // Функція рендерингу контенту стовпця (застаріла, замінена на getOrderColumns)
          ...
          <CardContent>
            {orders.length === 0 ? (
              ...
              <DataTable 
                ...
                loading={isLoading}
                storageKey="orders"
                onRowClick={(order) => handleEditOrder(order)}
                        ...
                        {order.items.map((item: any, index: number) => (
                          <div key={index} className="flex justify-between items-center p-3 bg-white rounded border">
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-gray-500">
                                Код: {item.productCode || item.itemCode || 'Не вказано'}
                              </div>
                            </div>
                            <div className="text-right">
                              <div className="font-medium">{item.quantity} {item.unit || 'шт'}</div>
                              <div className="text-sm text-gray-500">
                                {formatCurrency(parseFloat(item.price || '0'))} за одиницю
                              </div>
                            </div>
                ...
                expandedItems={expandedOrderId ? new Set([expandedOrderId]) : new Set()}
                onToggleExpand={(id) => toggleOrderExpansion(id)}
              />
            )}
          </CardContent>
        </Card>
