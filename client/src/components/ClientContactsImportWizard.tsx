...
import { CheckCircle, AlertCircle, Upload, FileText, UserPlus, ArrowRight } from 'lucide-react';
...
export function ClientContactsImportWizard() {
  ...
  // Статистика існуючих контактів
  const { data: importStats } = useQuery({
    queryKey: ['/api/import-stats/client-contacts'],
    enabled: isOpen,
  });
        ...
        try {
          ...
          if (statusResponse.status === 'completed' || statusResponse.status === 'failed') {
            ...
            if (statusResponse.status === 'completed') {
              ...
              // Оновлюємо список контактів після успішного імпорту
              setTimeout(() => {
          ...
          <div className="flex items-center justify-between">
            {[1, 2, 3, 4, 5].map((stepNum) => (
              <div key={stepNum} className="flex items-center">
                ...
                {stepNum < 5 && (
                  <div className={`w-12 h-0.5 ${step > stepNum ? 'bg-green-600' : 'bg-gray-200'}`} />
                )}
              </div>
            ))}
          </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Ласкаво просимо до майстра імпорту контактів
                </CardTitle>
                <CardDescription>
                  Цей майстер допоможе вам імпортувати контакти клієнтів з XML файлу у форматі DATAPACKET
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="bg-green-50 p-4 rounded-lg">
                  <h4 className="font-medium text-green-900 mb-2">Що вам потрібно знати:</h4>
                  <ul className="space-y-1 text-sm text-green-800">
                    <li>• XML файл повинен бути у форматі DATAPACKET</li>
                    <li>• Контакти будуть прив'язані до існуючих клієнтів</li>
                    <li>• Система автоматично знайде дублікати</li>
                ...
                {importStats && (
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="text-sm text-gray-700">
                      У системі зараз: <span className="font-medium">{importStats.existing_contacts}</span> контактів
                    </div>
                  </div>
                )}
          ...
          {step === 2 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Виберіть файл з контактами для імпорту
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                  ...
                  <input
                    ...
                    className="mt-4 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-green-50 file:text-green-700 hover:file:bg-green-100"
                  />
                </div>
