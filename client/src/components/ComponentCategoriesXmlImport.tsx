...
export function ComponentCategoriesXmlImport() {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return apiRequest('/api/component-categories/import-xml', {
        method: 'POST',
        body: formData,
      });
    },
    onSuccess: (data) => {
      ...
      toast({
        ...
        description: "Процес імпорту категорій компонентів запущено",
      });
    },
    onError: (error) => {
  ...
  const startPolling = (jobId: string) => {
    ...
    const pollStatus = async () => {
      try {
        const response = await apiRequest(`/api/component-categories/import-xml/${jobId}/status`);
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Імпорт категорій компонентів з XML
        </CardTitle>
        <CardDescription>
          Завантажте XML файл для імпорту категорій компонентів. Підтримувані поля: ID_GROUP (id), GROUP_NAME (name).
        </CardDescription>
      </CardHeader>
