...
interface SupplierReceiptsImportProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onImportComplete: () => void;
}

export function SupplierReceiptsImport({ open, onOpenChange, onImportComplete }: SupplierReceiptsImportProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await apiRequest('/api/import/supplier-receipts', {
        method: 'POST',
        headers: {
      ...
      toast({
        ...
        description: `Імпортовано ${response.imported} приходів з ${response.total} записів`
      });
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Імпорт приходів з XML
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <div>
            <Label htmlFor="file-upload">Оберіть XML файл з приходами</Label>
            <Input
              id="file-upload"
              type="file"
              accept=".xml"
