...
// Функція для HTTP-сервісу (POST обробник endpoint /invoices)
Процедура ОбработатьЗапросНакладных(Запрос, Ответ) Экспорт
    ...
    Попытка
        ...
        ЗаписатьВФайл("c:\temp\erp_debug.txt", 
            "Обробка запиту накладних, action: " + ПараметрыЗапроса.action);
        ...
        Иначе
            // Стандартний запит всіх накладних (існуюча логіка)
            МассивНакладных = ПолучитьВсеНакладные();
            ЗаписатьВФайл("c:\temp\erp_debug.txt", 
                "Успішно отримано " + Формат(МассивНакладных.Количество(), "ЧГ=0") + " всіх накладних");
        КонецЕсли;
