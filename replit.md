...
- ✅ **ВИПРАВЛЕНО КРИТИЧНУ ПОМИЛКУ "receipt_id" CONSTRAINT VIOLATION** - усунуто проблему з передачею позицій накладних через webhook
- ✅ **ОНОВЛЕНО СХЕМУ SUPPLIER_RECEIPT_ITEMS** - додано поля item_name, item_code, unit та зроблено componentId опціональним
- ✅ **ВИПРАВЛЕНО ВСІ WEBHOOK МЕТОДИ** - змінено supplierReceiptId на receiptId у всіх операціях з позиціями
- ✅ **ДОДАНО ПІДТРИМКУ ОДИНИЦЬ ВИМІРЮВАННЯ** - система тепер зберігає ЕдиницаИзмерения з 1С webhook
- ✅ **ДОДАНО КНОПКУ "ІМПОРТ З 1С" В ПРИХОДИ ПОСТАЧАЛЬНИКІВ** - створено Import1CInvoicesSimple компонент з повною функціональністю імпорту накладних
- ✅ **ІНТЕГРОВАНО ІСНУЮЧУ КНОПКУ** - замінено стандартну кнопку на функціональний компонент імпорту з 1С з фільтрами дат та вибором накладних
- ✅ **ВИПРАВЛЕНО ВСІ TYPESCRIPT ТА RUNTIME ПОМИЛКИ** - усунуто проблеми з DatePeriodFilter, Checkbox типами та схемою ProcessedInvoice1C
- ✅ **ДОДАНО ПЕРЕГЛЯД ПОЗИЦІЙ НАКЛАДНИХ** - реалізовано розгортані таблиці позицій з детальною інформацією про товари, ціни та статуси зіставлення
- ✅ **ПОКРАЩЕНО UX ІМПОРТУ** - додано кнопки перегляду позицій з іконками та анімацією розгортання/згортання для кожної накладної
