...
### July 19, 2025
- ✅ **КРИТИЧНА ПРОБЛЕМА ДУБЛІКАТІВ ЗАМОВЛЕНЬ ОСТАТОЧНО ВИРІШЕНА** - впроваджено upsert логіку в усі методи імпорту замовлень
- ✅ **ВИПРАВЛЕНО importOrdersFromXmlWithProgress** - тепер шукає існуючі замовлення за номером перед створенням нових
- ✅ **ВИПРАВЛЕНО importOrdersFromXml** - додано перевірку дублікатів та логіку оновлення замість створення
- ✅ **ВИПРАВЛЕНО BITRIX24 WEBHOOK** - система перевіряє існуючі замовлення за номером рахунку або ID перед створенням
- ✅ **ВИПРАВЛЕНО 1С WEBHOOK** - createOutgoingInvoiceFromWebhook тепер оновлює існуючі замовлення замість створення дублікатів
