...
- ❌ **1С СЕРВЕР ПОВЕРТАЄ HTTP 405 METHOD NOT ALLOWED** - проблема в налаштуваннях HTTP-сервісу 1С:
  - Авторизація не працює (показує "Анонимная" замість "Школа І.М.")
  - HTTP-сервіс не приймає POST запити на endpoint `/invoices`
  - Потрібно перевірити публікацію HTTP-сервісу та налаштування методів
