...
### July 14, 2025
- ✅ **КРИТИЧНУ ПРОБЛЕМУ AUTO-MAPPING В PREVIEW РЕЖИМІ ВИРІШЕНО ОСТАТОЧНО** - виявлено та усунуто корінь проблеми з неправильними зіставленнями
- ✅ **ВИЯВЛЕНО REAL ROOT CAUSE** - ComponentMappingCell автоматично викликає `/api/1c/invoices/check-mapping/` під час preview що створювало неправильні зіставлення
- ✅ **ПЕРЕПИСАНО API ENDPOINT** - змінено логіку /api/1c/invoices/check-mapping/:productName для "тільки перегляду" режиму
- ✅ **ВИДАЛЕНО АВТОМАТИЧНЕ СТВОРЕННЯ ЗІСТАВЛЕНЬ** - endpoint тепер ТІЛЬКИ перевіряє існуючі зіставлення в БД без створення нових
- ✅ **ДВОЕТАПНА ЛОГІКА ENDPOINT** - 1) Пошук готових зіставлень в product_name_mappings → 2) Якщо не знайдено: {"found":false}
- ✅ **ПРОТЕСТОВАНО ПОВНУ ФУНКЦІОНАЛЬНІСТЬ** - підтверджено правильну роботу для існуючих та неіснуючих зіставлень
- ✅ **УСУНУТО КАТЕГОРІЙНІ ПОМИЛКИ** - фрези більше НЕ зіставляються з метчиками, мікросхеми НЕ зіставляються з фрезами
- ✅ **СИСТЕМА ГОТОВА ДЛЯ PRODUCTION** - preview режим безпечний, створення зіставлень тільки при реальному імпорті накладних
