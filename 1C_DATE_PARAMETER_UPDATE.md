...
**CRITICAL**: ERP sends dates in ISO format "2025-07-17" but 1C cannot parse this format directly, causing error:
```
Ошибка парсинга параметров: Перетворення значення к типу Дата не може бути виконано
```

## Solution
1. Update the 1C HTTP service to accept and process date parameters from ERP requests
2. Add ISO date conversion function to handle "2025-07-17" format
3. Convert ISO dates to 1C Date() format: Дата(2025, 07, 17)
    ...
    Попытка
        ...
        Если НЕ ПустаяСтрока(ТелоЗапроса) Тогда
            Попытка
                ...
                // Обработка пользовательских дат (конвертация из ISO формата)
                Если ПараметрыЗапроса.Свойство("dateFrom") И НЕ ПустаяСтрока(ПараметрыЗапроса.dateFrom) Тогда
                    ДатаНачала = ПреобразоватьИЗОДатуВДату(ПараметрыЗапроса.dateFrom);
                КонецЕсли;
                
                Если ПараметрыЗапроса.Свойство("dateTo") И НЕ ПустаяСтрока(ПараметрыЗапроса.dateTo) Тогда
                    ДатаОкончания = КонецДня(ПреобразоватьИЗОДатуВДату(ПараметрыЗапроса.dateTo));
                КонецЕсли;
...
// Функция для преобразования ISO даты в формат 1C
Функция ПреобразоватьИЗОДатуВДату(ИЗОДата)
    
    // Формат: "2025-07-17" -> Дата(2025,07,17)
    Попытка
        МассивЧастей = СтрРазделить(ИЗОДата, "-");
