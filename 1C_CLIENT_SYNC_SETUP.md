...
Функция ПолучитьДанныеКомпании(Организация)
    // КРИТИЧНО: Дані компанії повинні передаватися з КОЖНИМ клієнтом для ідентифікації джерела
    ДанныеКомпании = Новый Структура;
    Если ЗначениеЗаполнено(Организация) Тогда
        ДанныеКомпании.Вставить("НашаКомпанія", Организация.Наименование);
        ДанныеКомпании.Вставить("КомпаніяЄДРПОУ", Организация.КодПоЕДРПОУ); // ЄДРПОУ вашої компанії
        ДанныеКомпании.Вставить("КомпаніяКод", Организация.Код);
    КонецЕсли;
    Возврат ДанныеКомпании;
КонецФункции

// Функция для отправки данных клиента (ОНОВЛЕНА - тепер передає дані компанії)
Функция ОтправитьДанныеКлиента(Контрагент, Действие)
    Попытка
        ...
        // ОБОВ'ЯЗКОВО: Додаємо дані компанії до кожного клієнта
        Попытка
            Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент.Ссылка, "Организация");
            Если НЕ ЗначениеЗаполнено(Организация) Тогда
                // Використовуємо основну організацію за замовчуванням
                Организация = Константы.ОсновнаяОрганизация.Получить();
            КонецЕсли;
            
            ДанныеКомпании = ПолучитьДанныеКомпании(Организация);
            Для Каждого КлючЗначение Из ДанныеКомпании Цикл
                ДанныеКлиента.Вставить(КлючЗначение.Ключ, КлючЗначение.Значение);
            КонецЦикла;
        Исключение
            ЗаписатьЛог("Помилка отримання даних компанії: " + ОписаниеОшибки());
        КонецПопытки;
